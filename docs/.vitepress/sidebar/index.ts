...
// 生成侧边栏 Tree 结构
function generateTree(directory, isParent = true) {
  // 定义空的树
  const tree: any[] = []
  // 读取目录下的文件
  ...
  files.forEach((file) => {
    ...
    if (stats.isDirectory()) {
      // 如果是一个目录，则递归遍历并生成子树
      const subTree = generateTree(filePath, false)
      // 获取当前目录下的子文件的数量
      ...
      if (isParent) {
        const CommonData = tree.find((item) => item.text === '通用')
        if (!CommonData) {
          tree.unshift({
            text: '通用',
            collapsed: true,
